...
import { isSessionUnlocked, getSessionPassToken } from "./lib/accessCode";
...
export default function App() {
  ...
  const performDownloadSpec = (prod: Product) => {
    if (prod.hasDatasheet || prod.datasheetFile) {
      const passToken = getSessionPassToken();
      const link = document.createElement("a");
      link.href = `/api/products/${prod.id}/datasheet${passToken ? `?passToken=${encodeURIComponent(passToken)}` : ""}`;
